    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 409 && data.details) {
          setDeleteError(
            `This challenge has ${data.details.submissions} submissions and ${data.details.rankRequirements} rank requirements. Deleting it will remove all related data.`
          );
          setIsDeleting(false);
          return;
        }
  ...
  return (
    <>
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete "{challenge.name}"?</AlertDialogTitle>
            <AlertDialogDescription>
              {deleteError ? (
