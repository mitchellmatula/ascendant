    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 409 && data.details) {
          ...
          setDeleteError(
            `This division has ${data.details.rankRequirements} rank requirements and ${data.details.rankThresholds} rank thresholds. Deleting it will remove all related data.`
          );
          setIsDeleting(false);
          return;
        }
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          <DropdownMenuItem asChild>
            <Link href={`/admin/divisions/${division.id}`}>Edit</Link>
          </DropdownMenuItem>
          <DropdownMenuSeparator />
          <DropdownMenuItem
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {deleteError ? (
                ...
                "This action cannot be undone. This will permanently delete the division and all associated data."
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
